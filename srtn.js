function srtn() {
    ...
    for (i = 0; i < horaTermino[counter - 2] * counter; i++) {
        for (j = 1; j <= counter; j++) {
            if (tChegadas[j] == i) {
                if (( processos[processos.length - 1][2] - i) > tDuracoes[j]) {
                    fila.push([processos[processos.length - 1][0], processos[processos.length - 1][2], processos[processos.length - 1][1] + processos[processos.length - 1][2] - i]);
